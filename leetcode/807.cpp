// 给你一座由 n x n 个街区组成的城市，每个街区都包含一座立方体建筑。给你一个下标从 0
// 开始的 n x n 整数矩阵 grid ，其中 grid[r][c] 表示坐落于 r 行 c 列的建筑物的 高度 。
//
...
int main() {
    ...
    // 示例 1
    {
        ...
        check("示例1: grid=[[3,0,8,4],[2,4,5,7],[9,2,6,3],[0,3,1,0]]", result,
              35);
    }
    ...
    // 示例 2
    {
        ...
        int result = solution.maxIncreaseKeepingSkyline(grid);
        check("示例2: grid=[[0,0,0],[0,0,0],[0,0,0]]", result, 0);
    }
