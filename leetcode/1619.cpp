...
public:
    double trimMean(vector<int>& arr) {
        int remove_len = arr.size() / 20;
        sort(arr.begin(), arr.end());

        return (double)accumulate(arr.begin() + remove_len,
                                  arr.end() - remove_len, 0) /
               (arr.size() - 2 * remove_len);
    }
};
...
int main() {
    ...
    // 示例 1
    {
        vector<int> arr = {1, 2, 2, 2, 2, 2, 2, 2, 2, 2,
                           2, 2, 2, 2, 2, 2, 2, 2, 2, 3};
        auto result = solution.trimMean(arr);
        check("示例1", result, 2.0);
    }
